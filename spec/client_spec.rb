  ...
  context 'Document' do
    before :each do
      def check_async_response_format(response, options = {})
        expect(response.keys).to match_array(["receipts", "batch_link"])
        expect(response["receipts"]).to be_a_kind_of(Array)
        expect(response["receipts"].first.keys).to match_array(["id", "external_id", "link", "status", "errors"])
        expect(response["receipts"].first["external_id"]).to eq(options[:external_id]) if options[:external_id]
        expect(response["receipts"].first["status"]).to eq(options[:status]) if options[:status]
        expect(response["receipts"].first["errors"]).to eq(options[:errors]) if options[:errors]
      end
    end
    ...
    context '#document_receipts' do
      before :each do
        def get_receipt_ids
          ...
          VCR.use_cassette(:async_create_or_update_document_success) do
            response = client.async_create_or_update_documents(engine_slug, document_type_slug, documents)
            receipt_ids = response["receipts"].map { |r| r["id"] }
          end
          receipt_ids
        end
      end
